...
patientRouter.post('/loginPatient', async (req, res) => {
	try {
		...
		// Defensive check: ensure a stored password exists before comparing
		if (!patient.password) {
			console.error('loginPatient: found user record but no stored password for email:', email);
		...
		console.error('loginPatient error:', error && error.stack ? error.stack : error);
		return res.status(500).json({ message: 'Internal server error' });
	}
});
